...
# Track active flight tasks per order
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        # Cancel any active flight tasks for this client
...
async def handle_start_tracking(client_id: str, websocket: WebSocket, message: dict):
    """Handle start_tracking message from client"""
    ...
    # Cancel previous flight task if exists
    if client_id in active_flights:
        active_flights[client_id].cancel()

    # Start the flight simulation task
...
async def simulate_flight(
    client_id: str, websocket: WebSocket, order_id: str, start_lat: float, start_lng: float, end_lat: float, end_lng: float
):
    """
    Simulate drone flight by sending coordinates every 5 seconds.
    Total flight time is 60 seconds.
    """
    flight_duration = 60.0  # Total flight time in seconds
    ...
    try:
        while True:
            ...
            # Send coordinate update to client
            ...
            # If delivery is complete, break the loop
            if delivered:
